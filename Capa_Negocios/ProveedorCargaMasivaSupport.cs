...
namespace Capa_Negocios
{
    ...
    internal static class ProveedorCargaMasivaParser
    {
        ...
        public static byte[] GenerarExcelDesdePlantilla(byte[] plantilla)
        {
            ...
            {
                throw new Exception("No se encontro la plantilla base de proveedores.");
            }
            ...
            using (var memory = new MemoryStream())
            {
                ...
                using (var zip = new ZipArchive(memory, ZipArchiveMode.Update, true))
                {
                    ...
                    var workbookEntry = zip.GetEntry("xl/workbook.xml")
                        ?? throw new Exception("La plantilla de proveedores no contiene workbook.xml.");
                    var workbookRelsEntry = zip.GetEntry("xl/_rels/workbook.xml.rels")
                        ?? throw new Exception("La plantilla de proveedores no contiene workbook.xml.rels.");
                    ...
                    var firstSheet = workbookDoc.Descendants(ns + "sheet").FirstOrDefault()
                        ?? throw new Exception("La plantilla de proveedores no contiene hojas.");
                    ...
                    {
                        throw new Exception("No fue posible localizar la primera hoja de la plantilla de proveedores.");
                    }

                    string sheetPath = "xl/" + target.TrimStart('/').Replace("\\", "/");
                    var sheetEntry = zip.GetEntry(sheetPath)
                        ?? throw new Exception("No fue posible abrir la hoja principal de la plantilla de proveedores.");
                    ...
                    if (worksheet == null)
                    {
                        throw new Exception("La hoja principal de la plantilla de proveedores no tiene una estructura valida.");
                    }
                    ...
                    sheetData.Add(CrearFilaWorksheet(ns, 1, new[] { "prov_id", "nombre", "estado" }));
